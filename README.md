...
1、目前内置了fastDfs和Minio，定义了AbstractUpload类作为模板类使用

2、使用方法：在pom.xml中添加依赖，在application.yml中配置数据库连接信息，配置如下
```properties
#默认值为redis_single,可省略
#zy.upload.type=fastdfs
#zy.upload.fastdfs.http-anti-steal-token=true
#zy.upload.fastdfs.tracker-server=192.168.136.135:22122
#zy.upload.fastdfs.http-secret-key=FastDFS1234567890
#spring.servlet.multipart.max-file-size=10MB
#zy.upload.fastdfs.show-file=true
#zy.upload.fastdfs.nginx-prefix=http://192.168.136.135:8888
zy.upload.type=minio
zy.upload.minio.bucket-name=files
zy.upload.minio.end-point=http://192.168.136.135:9002
zy.upload.minio.access-key=minio
zy.upload.minio.secret-key=minioadmin
```